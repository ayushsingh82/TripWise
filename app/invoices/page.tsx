      ...
      <div className="pt-32 pb-20 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-8">
            <h2 className="text-4xl font-black text-[#0048E0] mb-4">Usage Dashboard</h2>
            <p className="text-lg text-black">Monitor your x402 limits, usage, and guardrail status</p>
          </div>

          {/* STATS SUMMARY */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-white border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] p-6 rounded-lg">
              <p className="text-sm font-bold text-black/60 mb-1">Total Limits</p>
              <p className="text-3xl font-black text-[#0048E0]">{limits.length}</p>
            </div>
            <div className="bg-white border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] p-6 rounded-lg">
              <p className="text-sm font-bold text-black/60 mb-1">Active</p>
              <p className="text-3xl font-black text-green-500">{limits.filter(l => l.status === 'active').length}</p>
            </div>
            <div className="bg-white border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] p-6 rounded-lg">
              <p className="text-sm font-bold text-black/60 mb-1">Warning</p>
              <p className="text-3xl font-black text-yellow-500">{limits.filter(l => l.status === 'warning').length}</p>
            </div>
            <div className="bg-white border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] p-6 rounded-lg">
              <p className="text-sm font-bold text-black/60 mb-1">Exceeded</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredLimits.map((limit) => (
              ...
              >
                ...
                {/* LIMIT VALUE */}
                <div className="mb-4">
                  <p className="text-sm text-black/60 mb-1">Limit</p>
                ...
                <div className="mb-4">
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-black/60">Current Usage</span>
                    <span className="font-bold text-black">{formatCurrentUsage(limit)}</span>
                  </div>
                ...
                {/* API ENDPOINT */}
                <div className="mb-4">
                  <p className="text-xs font-bold text-black/60 mb-1">Endpoint</p>
                  <p className="text-sm text-black font-mono bg-gray-100 px-2 py-1 rounded">
          ...
          {filteredLimits.length === 0 && (
            <div className="text-center py-12">
              <p className="text-xl font-bold text-black/60">No limits found</p>
              <Link href="/upload">
          ...
          <div className="mt-12 bg-white border-2 border-[#0048E0] shadow-[4px_4px_0_0_rgba(0,0,0,1)] p-6 rounded-lg max-w-3xl mx-auto">
            <h3 className="text-lg font-black text-[#0048E0] mb-3">Understanding Status</h3>
