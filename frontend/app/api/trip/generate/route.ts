...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { prompt, from, to, dates, budget } = body;
    ...
    trace.update?.({ input: { prompt: input } });

    const fullPrompt = TRIP_PLAN_PROMPT.replace("{{input}}", input);
    ...
    trace.update?.({ output: { tripId, itemCount: items.length } });
    trace.end?.();
